(ns optimizely.main
  "Kotodama WASM entrypoint for the Optimizely clean-room actor (L4) — Clojure port.
  ...
  facts (`optimizely.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "optimizely")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"optimizely.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "optimizely-compat" :tier tier :entities entities} 200])
...
(defn start! [] :optimizely-compat/ready)
